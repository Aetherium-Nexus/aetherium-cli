...
describe("aetherium warp apply config extension tests", async function () {
  ...
  it("should update destination gas configuration", async () => {
    ...
    // Apply the changes
    await aetheriumWarpApply(warpDeployPath, WARP_CORE_CONFIG_PATH_2);
  ...
  it("should update remote routers configuration", async () => {
    ...
    expect(
      updatedConfig[CHAIN_NAME_2].remoteRouters![chain3Id].address.toLowerCase()
  ...
  it("should preserve deploy config when extending warp route", async () => {
    ...
    const extendedConfig: AetTokenRouterConfig = {
      decimals: 18,
      mailbox: chain2Addresses!.mailbox,
      name: "Ether",
      owner: new Wallet(ANVIL_KEY).address,
