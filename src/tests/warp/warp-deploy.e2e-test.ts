...
import { ERC20Test, ERC4626Test } from "@aetherium-nexus/core";
...
describe("aetherium warp deploy e2e tests", async function () {
  ...
  describe("aetherium warp deploy --config ...", () => {
    it(`should exit early when the provided deployment file does not exist`, async function () {
      ...
      const output = aetheriumWarpDeployRaw({
  ...
  describe("aetherium warp deploy --config ... --yes", () => {
  ...
  describe(`aetherium warp deploy --config ... --yes --key ...`, () => {
    ...
    it("should only allow rebasing yield route to be deployed with rebasing synthetic", async function () {
      ...
      writeYamlOrJson(WARP_DEPLOY_OUTPUT_PATH, warpConfig);
      await aetheriumWarpDeploy(WARP_DEPLOY_OUTPUT_PATH).should.be.rejected; // TODO: revisit this to figure out how to parse the error.
    });
    ...
    it("should deploy with an ISM config", async () => {
      ...
      writeYamlOrJson(WARP_DEPLOY_OUTPUT_PATH, warpConfig);
      await aetheriumWarpDeploy(WARP_DEPLOY_OUTPUT_PATH);
    ...
    it("should send a message from origin to destination in the correct order", async function () {
      ...
      const { stdout: chain2Tochain3Stdout } = await aetheriumWarpSendRelay(
        CHAIN_NAME_2,
        CHAIN_NAME_3,
        WARP_CORE_CONFIG_PATH_2_3
      );
      ...
      const { stdout: chain3Tochain2Stdout } = await aetheriumWarpSendRelay(
        CHAIN_NAME_3,
        CHAIN_NAME_2,
        WARP_CORE_CONFIG_PATH_2_3
      );
