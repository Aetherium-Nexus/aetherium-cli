...
import { ERC20Test } from "@aetherium-nexus/core";
...
import { Address, assert, deepCopy, randomInt } from "@aetherium-nexus/utils";
...
import {
  aetheriumWarpApply,
  aetheriumWarpCheck,
  aetheriumWarpCheckRaw,
  aetheriumWarpDeploy,
} from "../commands/warp.js";

describe("aetherium warp check e2e tests", async function () {
  ...
  describe("aetherium warp check --symbol ... --config ...", () => {
    it(`should not find any differences between the on chain config and the local one`, async function () {
      ...
      const output = aetheriumWarpCheckRaw({
        symbol: tokenSymbol,
        warpDeployPath: WARP_DEPLOY_OUTPUT_PATH,
      })
        .stdio("pipe")
  ...
  describe("aetherium warp check --symbol ... --config ... --key ...", () => {
    it(`should not find any differences between the on chain config and the local one`, async function () {
      await deployAndExportWarpRoute();

      const output = await aetheriumWarpCheck(
        WARP_DEPLOY_OUTPUT_PATH,
        tokenSymbol
      );
