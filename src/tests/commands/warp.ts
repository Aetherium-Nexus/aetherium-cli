 ...
 */
export function aetheriumWarpInit(warpCorePath: string): ProcessPromise {
  return $`yarn workspace @aetherium-nexus/cli run aetherium warp init \
 ...
 * Deploys the Warp route to the specified chain using the provided config.
 */
export function aetheriumWarpDeployRaw({
  warpCorePath,
  aetKey,
  ...
  return $`${
    aetKey ? ["AET_KEY=" + aetKey] : ""
  } yarn workspace @aetherium-nexus/cli run aetherium warp deploy \
        --registry ${REGISTRY_PATH} \
 ...
 * Deploys the Warp route to the specified chain using the provided config.
 */
export function aetheriumWarpDeploy(warpCorePath: string): ProcessPromise {
  return aetheriumWarpDeployRaw({
 ...
 */
export async function aetheriumWarpApply(
  warpDeployPath: string,
  warpCorePath: string,
  strategyUrl = ""
) {
  return $`yarn workspace @aetherium-nexus/cli run aetherium warp apply \
        --registry ${REGISTRY_PATH} \
...
export function aetheriumWarpReadRaw({
  chain,
  warpAddress,
  outputPath,
  symbol,
  ...
  return $`yarn workspace @aetherium-nexus/cli run aetherium warp read \
        --registry ${REGISTRY_PATH} \
...
export function aetheriumWarpRead(
  chain: string,
  warpAddress: string,
  warpDeployPath: string
): ProcessPromise {
  return aetheriumWarpReadRaw({
    chain,
    warpAddress,
...
export function aetheriumWarpCheckRaw({
  warpDeployPath,
  symbol,
}: {
  symbol?: string;
  ...
  return $`yarn workspace @aetherium-nexus/cli run aetherium warp check \
        --registry ${REGISTRY_PATH} \
        ${symbol ? ["--symbol", symbol] : ""} \
        --verbosity debug \
...
export function aetheriumWarpCheck(
  warpDeployPath: string,
  symbol: string
): ProcessPromise {
  return aetheriumWarpCheckRaw({
    warpDeployPath,
    symbol,
  });
}

export function aetheriumWarpSendRelay(
  origin: string,
  destination: string,
  warpCorePath: string,
  relay = true,
  ...
  return $`yarn workspace @aetherium-nexus/cli run aetherium warp send \
        ${relay ? "--relay" : ""} \
        --registry ${REGISTRY_PATH} \
        --origin ${origin} \
...
export async function readWarpConfig(
  ...
  await aetheriumWarpRead(chain, warpAddress!, warpDeployPath);
...
function getWarpTokenConfigForType({
  ...
  tokenType,
  vault,
}: GetWarpTokenConfigByTokenTypeOptions): AetTokenRouterConfig {
  let tokenConfig: AetTokenRouterConfig;
  switch (tokenType) {
