...
import { Address, sleep } from "@aetherium-nexus/utils";
...
import { aetheriumCoreDeploy } from "./core.js";
import {
  aetheriumWarpApply,
...
export async function updateOwner(
  ...
  return aetheriumWarpApply(warpConfigPath, warpCoreConfigPath);
}
...
export async function extendWarpConfig(params: {
  ...
  extendedConfig: AetTokenRouterConfig;
  warpCorePath: string;
  warpDeployPath: string;
  strategyUrl?: string;
}): Promise<string> {
  ...
  await aetheriumWarpApply(warpDeployPath, warpCorePath, strategyUrl);
...
export async function setupIncompleteWarpRouteExtension(
  ...
  warpConfigPath: string;
  configToExtend: AetTokenRouterConfig;
  ...
  const configToExtend: AetTokenRouterConfig = {
    decimals: 18,
...
export async function deployOrUseExistingCore(
  ...
  if (!addresses) {
    await aetheriumCoreDeploy(chain, coreInputPath);
...
export async function sendWarpRouteMessageRoundTrip(
  ...
  await aetheriumWarpSendRelay(chain1, chain2, warpCoreConfigPath);
  return aetheriumWarpSendRelay(chain2, chain1, warpCoreConfigPath);
}

export async function aetheriumSendMessage(
  origin: string,
  destination: string
) {
  return $`yarn workspace @aetherium-nexus/cli run aetherium send message \
...
export function aetheriumRelayer(chains: string[], warp?: string) {
  return $`yarn workspace @aetherium-nexus/cli run aetherium relayer \
        --registry ${REGISTRY_PATH} \
        --chains ${chains.join(",")} \
        --warp ${warp ?? ""} \
        --key ${ANVIL_KEY} \
