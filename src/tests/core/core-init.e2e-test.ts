...
import { Address, normalizeAddress } from "@aetherium-nexus/utils";

import { readYamlOrJson } from "../../utils/files.js";
import { aetheriumCoreInit } from "../commands/core.js";
import {
  ANVIL_KEY,
...
describe("aetherium core init e2e tests", async function () {
  ...
  describe("aetherium core init", () => {
    it("should successfully generate the core contract deployment config", async () => {
      const output = aetheriumCoreInit(CORE_CONFIG_PATH_2).stdio("pipe");
  ...
  describe("AET_KEY=... aetherium core init", () => {
    it("should successfully generate the core contract deployment config when confirming owner prompts", async () => {
      ...
      const output = aetheriumCoreInit(
        CORE_CONFIG_PATH_2,
        undefined,
        ANVIL_KEY
      ).stdio("pipe");
  ...
  describe("aetherium core init --key ...", () => {
    it("should successfully generate the core contract deployment config when confirming owner prompts", async () => {
      const owner = new Wallet(ANVIL_KEY).address;
      const steps: TestPromptAction[] = [
        CONFIRM_DETECTED_OWNER_STEP,
      ...
      const output = aetheriumCoreInit(CORE_CONFIG_PATH_2, ANVIL_KEY).stdio(
        "pipe"
      );
