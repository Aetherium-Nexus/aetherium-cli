...
import { randomAddress } from "@aetherium-nexus/sdk";
...
describe("aetherium core check e2e tests", async function () {
  ...
  it("should throw an error if the --chain param is not provided", async () => {
    const wrongCommand =
      $`yarn workspace @aetherium-nexus/cli run aetherium core check \
  ...
  it("should successfully run the core check command", async () => {
    ...
    const output = await aetheriumCoreCheck(
      CHAIN_NAME_2,
      CORE_READ_CONFIG_PATH_2
    );
