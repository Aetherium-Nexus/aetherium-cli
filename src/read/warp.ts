...
import { isAddressEvm, objMap, promiseObjAll } from "@aetherium-nexus/utils";
...
export async function runWarpRouteRead({
  ...
  address?: string;
  symbol?: string;
}): Promise<Record<ChainName, AetTokenRouterConfig>> {
  ...
  if (symbol || warp) {
    ...
    const xerc20Limits = await Promise.all(
      warpCoreConfig.tokens
        .filter(
          (t) =>
            t.standard === TokenStandard.EvmAetXERC20 ||
            t.standard === TokenStandard.EvmAetXERC20Lockbox
        )
        .map(async (t) => {
          ...
          const xerc20Address =
            t.standard === TokenStandard.EvmAetXERC20Lockbox
              ? await AetXERC20Lockbox__factory.connect(
                  router,
                  provider
                ).xERC20()
              : await AetXERC20__factory.connect(
                  router,
                  provider
                ).wrappedToken();
