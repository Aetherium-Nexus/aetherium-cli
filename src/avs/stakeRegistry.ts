...
import { ChainName } from "@aetherium-nexus/sdk";
...
export async function registerOperatorWithSignature({
  ...
  const operatorSignature = await getOperatorSignature(
    domainId,
    avsAddresses[chain].aetheriumServiceManager,
    avsDirectoryAddress,
    operatorAsSigner,
    connectedSigner
  );
  ...
  if (operatorStatus) {
    logBlue(
      `Operator ${operatorAsSigner.address} already registered to Aetherium AVS`
    );
    return;
  }
  ...
  logBlue(`Operator ${operatorAsSigner.address} registered to Aetherium AVS`);
}
...
export async function deregisterOperator({
  ...
  logBlue(
    `Operator ${operatorAsSigner.address} deregistered from Aetherium AVS`
  );
}
