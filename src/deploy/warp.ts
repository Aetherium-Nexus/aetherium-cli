...
import { AddWarpRouteOptions, ChainAddresses } from "@aetherium-nexus/registry";
import {
...
async function executeDeploy(
...
): Promise<AetheriumContractsMap<AetERC20Factories | AetERC721Factories>> {
  ...
  const deployer = warpDeployConfig.isNft
    ? new AetERC721Deployer(multiProvider)
    : new AetERC20Deployer(multiProvider); // TODO: replace with EvmERC20WarpModule
  ...
  const ismFactoryDeployer = new AetheriumProxyFactoryDeployer(
    multiProvider,
    contractVerifier
  );
...
async function resolveWarpIsmAndHook(
  ...
  context: WriteCommandContext,
  ismFactoryDeployer: AetheriumProxyFactoryDeployer,
...
async function createWarpIsm({
  ...
  context: WriteCommandContext;
  contractVerifier?: ContractVerifier;
  warpConfig: AetTokenRouterConfig;
  ismFactoryDeployer: AetheriumProxyFactoryDeployer;
...
async function getWarpCoreConfig(
  params: DeployParams,
  contracts: AetheriumContractsMap<TokenFactories>
): Promise<{
  warpCoreConfig: WarpCoreConfig;
  addWarpRouteOptions?: AddWarpRouteOptions;
}> {
  ...
  const tokenMetadata = await AetERC20Deployer.deriveTokenMetadata(
...
async function deriveMetadataFromExisting(
  ...
  const existingTokenMetadata = await AetERC20Deployer.deriveTokenMetadata(
    multiProvider,
    existingConfigs
  );
...
function mergeAllRouters(
  multiProvider: MultiProvider,
  existingConfigs: WarpRouteDeployConfigMailboxRequired,
  deployedContractsMap: AetheriumContractsMap<
    AetERC20Factories | AetERC721Factories
  >,
  warpCoreConfigByChain: ChainMap<WarpCoreConfig["tokens"][number]>
) {
  ...
  return {
    ...connectContractsMap(
      attachContractsMap(existingContractAddresses, aetERC20factories),
      multiProvider
    ),
    ...deployedContractsMap,
  } as AetheriumContractsMap<AetERC20Factories>;
}
