...
import { ChainMap, MultisigIsmConfig, ZHash } from "@aetherium-nexus/sdk";
import {
  Address,
  isValidAddress,
  normalizeAddressEvm,
...
export async function createMultisigConfig({
  ...
  log(
    "Select your own chain below to run your own validators. If you want to reuse existing Aetherium validators instead of running your own, do not select additional mainnet or testnet chains."
  );
  ...
  for (const chain of chains) {
    ...
    if (Object.keys(chainAddresses).includes(chain)) {
      const reuseCoreConfig = await confirm({
        message: "Use existing Aetherium validators for this chain?",
      });
  ...
  if (isValidMultisigConfig(result)) {
    ...
    errorRed(
      `Multisig config is invalid, please see https://github.com/Aetherium-Nexus/aetherium-cli/blob/main/examples/ism.yaml for an example`
    );
