...
export function readChainConfigs(filePath: string) {
  ...
  if (!parseResult.success) {
    errorRed(
      `Chain config for ${filePath} is invalid, please see https://github.com/Aetherium-Nexus/aetherium-cli/blob/main/examples/chain-config.yaml for an example`
    );
...
export async function createChainConfig({
  ...
  if (parseResult.success) {
    ...
    errorRed(
      `Chain config is invalid, please see https://github.com/Aetherium-Nexus/aetherium-cli/blob/main/examples/chain-config.yaml for an example`
    );
    errorRed(JSON.stringify(parseResult.error.errors));
...
async function addBlockExplorerConfig(metadata: ChainMetadata): Promise<void> {
  ...
  if (wantBlockExplorerConfig) {
    ...
    const apiKey =
      (await input({
        message:
          "Optional: Provide an API key for the explorer, or press 'enter' to skip. Please be sure to remove this field if you intend to add your config to the Aetherium registry:",
      })) ?? undefined;
