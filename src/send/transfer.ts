...
import { parseWarpRouteMessage, timeout } from "@aetherium-nexus/utils";
...
async function executeDelivery({
  ...
  const core = AetheriumCore.fromAddressesMap(chainAddresses, multiProvider);
  ...
  const message: DispatchedMessage =
    AetheriumCore.getDispatchedMessages(transferTxReceipt)[messageIndex];
  ...
  if (selfRelay) {
    const relayer = new AetheriumRelayer({ core });
