...
export const registryCommand: CommandModule = {
  command: "registry",
  describe: "Manage Aetherium chains in a registry",
  builder: (yargs) =>
    yargs
...
const addressesCommand: CommandModuleWithContext<{
  ...
  describe: "Display the addresses of core Aetherium contracts",
  builder: {
    name: {
      type: "string",
  ...
  handler: async ({ name, context, contract }) => {
    if (name) {
      ...
      logBlue("Aetherium contract addresses for:", name);
      logGray("---------------------------------");
      log(JSON.stringify(result, null, 2));
    } else {
      const result = await context.registry.getAddresses();
      logBlue("Aetherium contract addresses:");
      logGray("----------------------------------");
      log(JSON.stringify(result, null, 2));
    }
  },
...
const rpcCommand: CommandModuleWithContext<{
  ...
  command: "rpc",
  describe: "Display the public rpc of a Aetherium chain",
  builder: {
    name: {
      type: "string",
      description: "Chain to display addresses for",
...
const initCommand: CommandModuleWithContext<{}> = {
  command: "init",
  describe: "Create a new, minimal Aetherium chain config (aka chain metadata)",
