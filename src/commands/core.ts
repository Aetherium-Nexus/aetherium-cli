...
import { diffObjMerge } from "@aetherium-nexus/utils";
...
export const coreCommand: CommandModule = {
  command: "core",
  describe: "Manage core Aetherium contracts & configs",
  builder: (yargs) =>
    yargs
      .command(apply)
      .command(check)
...
export const apply: CommandModuleWithWriteContext<{
  ...
  handler: async ({ context, chain, config: configFilePath }) => {
    logCommandHeader(`Aetherium Core Apply`);
...
/**
 * Generates a command module for deploying Aetherium contracts, given a command
 *
 * @param commandName - the deploy command key used to look up the deployFunction
 * @returns A command module used to deploy Aetherium contracts.
 */
export const deploy: CommandModuleWithWriteContext<{
  ...
  command: "deploy",
  describe: "Deploy Aetherium contracts",
  builder: {
  ...
  handler: async ({ context, chain, config: configFilePath, dryRun }) => {
    logCommandHeader(`Aetherium Core deployment${dryRun ? " dry-run" : ""}`);
...
export const init: CommandModuleWithContext<{
  ...
  handler: async ({ context, advanced, config: configFilePath }) => {
    logCommandHeader("Aetherium Core Configure");
...
export const read: CommandModuleWithContext<{
  ...
  handler: async ({ context, chain, mailbox, config: configFilePath }) => {
    logCommandHeader("Aetherium Core Read");
...
export const check: CommandModuleWithContext<{
  ...
  handler: async ({ context, chain, mailbox, config: configFilePath }) => {
    logCommandHeader("Aetherium Core Check");
